...
class ZeddemoreProgressBar {
    ...
    #barEmptyColor = c.DEFAULT_BAR_EMPTY_COLOR;
    #barGlue = '';
    ...
    constructor(options) {
        ...
        this.#caller = options.caller || options.user || null;
    ...
    set barEmptyColor(color) {
        if (_.isString(color) && (color.length > 0)) this.#barEmptyColor = color;
    }
    ...
    #formatBar(progress, completed, incompleted, size = this.barSize, glue = this.barGlue) {
        ...
        const bar = completed.substring(0, completeSize) + glue + incompleted.substring(0, incompleteSize);
        const color = this.isEmpty ? this.barEmptyColor : this.barGradient[ barGradientIndex ];
        return chalk.hex(color)(bar);
    }
    ...
    #formatPercentage(percentage) {
        const _percentage = percentage.toString() + '%';
        return this.autoPadding ? _percentage.padStart(4, ' ') : _percentage;
    }
    ...
    #formatValue(value, total) {
        ...
        return _value.padStart(_total.length, ' ');
    }
    ...
    #formatter(options, params, payload) {
        ...
        if (this.showBar) calculatedPayload.bar = this.#formatBar(progress, barCompleteString, barIncompleteString);
