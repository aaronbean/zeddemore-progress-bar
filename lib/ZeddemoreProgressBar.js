...
class ZeddemoreProgressBar {
    ...
    #formatBar(progress, completed, incompleted, size, glue, chaser = null, chaserOffset = 0) {
        ...
        const glueColor = this.barGlueColor;
        const incompleteColor = this.isEmpty ? this.barEmptyColor : this.barIncompleteCharacterColor;
        if (this.useCompletionGradient && !this.hasChaser && !this.isEmpty) {
        ...
        if (_.isString(chaser) && !!chaser.length && (completeSize > chaserOffset)) {
            let barChaser = chaser;
            let barCompleted1 = completed.substring(0, completeSize - chaser.length - chaserOffset);
            let barCompleted2 = completed.substring(completeSize - chaser.length - chaserOffset, completeSize);
            ...
            if (isColor(this.barChaserColor)) barChaser = chalk.hex(this.barChaserColor)(barChaser);
    ...
    #formatter(options, params, payload) {
        ...
        const barSize = processing ? (this.barSize - this.barChaser.length - this.barGlue.length) : this.barSize;
        const barGlue = processing ? this.barGlue : '';
        const barChaser = processing ? (this.hasChaser ? this.barChaser : '') : '';
        let barChaserOffset = this.barChaserOffset;
        if (this.hasChaser) barChaserOffset -= Math.floor(progress * barChaserOffset);
        if (this.showBar) calculatedPayload.bar = this.#formatBar(progress, barCompleteString, barIncompleteString, barSize, barGlue, barChaser, barChaserOffset);
